...
### 6.22 Contribuição pública de espécie não pode tocar proveniência — decisão de 05/08/2026
...
| Data | Alteração |
|---|---|
| 05/08/2026 | **§6.22 — a nova contribuição pública de espécie não pode tocar proveniência.** O site ganhou login e um formulário para conta aprovada propor espécie nova ou edição (fica pendente até um master aceitar). A lista branca de campos aceitos — `nome_cientifico`, `nome_comum`, `tipo`, `descricao`, `credito_imagem`, `fonte_imagem_url`, `fonte_url`, `locais` — **exclui `iucn_categoria`, `iucn_avaliado_em`, `iucn_versao`, `iucn_taxon_id`, `fonte_iucn_url`, `aphia_id` e `gbif_key` para todo mundo, master incluso**: são os mesmos campos que a migração `0022` (§2.4, abaixo) passou a exigir com data e origem depois do episódio de `Mussismilia braziliensis`. Um formulário público moderado ainda é uma porta para categoria digitada sem procedência — a aprovação audita quem editou, não se o dado tem lastro. Enviar um desses campos recusa com `400` nomeando o campo, não ignora em silêncio. |
